...
export default function StickyNote({ node, selected, onStartDrag, onStartResize }: NodeComponentProps) {
  const viewport = useCanvasStore((state: CanvasStoreState) => state.viewport);
  ...
  return (
    <div
      className={`node sticky-note ${selected ? "selected" : ""}`}
      style={{
        left: node.x,
        top: node.y,
